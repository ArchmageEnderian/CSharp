...
namespace _2_Инда__Жуков
{
    class Program
    {
        ...
        static void Aktiv(List<int> listing, int[,] matrix, string[] range, bool[] on_off)
        {
        ...
        static void Main(string[] args)
        {
            ...
            {
                values[0, i] = i;
                values[1, i] = degre[i] * Convert.ToInt32(range[i]);
            }
            ...
            for (int i = 0; i < n; i++)
            {
                int vertex = values[0, i];
                list_on.Clear();
                list_on.Add(vertex);
                if(!on_or_off[vertex])
                {
                    rez.Add(vertex);
                    Aktiv(list_on, matrix, range, on_or_off);
                }
            }
